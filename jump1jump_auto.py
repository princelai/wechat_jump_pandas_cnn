...
class J1J:
    ...
    def handle_pic(self):
        ...
        person_head_color = np.array([ 52,  53, 61])
        person_bottom_diff = pd.DataFrame(np.sqrt(np.square(img_partial1 - person_bottom_color).sum(axis=2)))
        ...
        person_head_diff = pd.DataFrame(np.sqrt(np.square(img_partial1 - person_head_color).sum(axis=2)))
